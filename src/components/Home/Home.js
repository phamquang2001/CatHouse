...
function Home(props) {
    ...
    const [displayTimeFlashSale, setdisplayTimeFlashSale] = useState("00 giờ 00 phút 00 giây")
    const newDate = new Date()

    const countTimeFlashSale = () => {
        let timeCurrent = newDate.getHours() * 60 * 60 + newDate.getMinutes() * 60 + newDate.getSeconds()
    ...
    useEffect(() => {
        ...
        const h = Math.floor(timeFlashSale / 3600).toString().padStart(2, '0'),
            m = Math.floor(timeFlashSale % 3600 / 60).toString().padStart(2, '0'),
            s = Math.floor(timeFlashSale % 60).toString().padStart(2, '0');
        ...
        <div className="homePage">
            ...
            <div className="homePage-list-product" >
                <h5 className="header-slick container pt-2 d-flex">
                    <div>
                        <span className="spinner-grow text-warning spinner-grow-sm me-2" role="status" ></span>
                        <span>KHUYẾN MÃI</span>
                    </div>
                    <div>
                        <span className="ms-3" style={{ color: "yellow" }}>{displayTimeFlashSale}</span>
                    </div>
                </h5>
                ...
                <div className="btn btn-light mt-1" onClick={() => navigate("/collections/Mèo")}>
                    Xem tất cả
            ...
            <div className="homePage-list-category" >
                <div>
                    <h4 className="text-left p-2 pt-5  " style={{ fontWeight: "800" }}>TOP SẢN PHẨM MỚI</h4>
                </div>
                <SwiperProduct
                    initIndex={0}
                    lastIndex={21}
