...
import { getOneUser, getSearchOrder, putCartOrder, putUpdateProduct } from '../services/apiServices';
import _ from "lodash"
import { v4 as uuidv4 } from 'uuid';
import { putInfoUserAction, putUpdateInfoUserRedux, USER_LOGIN } from '../../redux/actions/userActions';
...
function UserListOrder(props) {
    ...
    const handeDeleteState = (event) => {
        ...
        let indexOrderState = infoUser.listOrder.findIndex(item => item.id === event.id)
                                                                ...
                                                                <div key={uuidv4().toString()} className='user-order-content'>
                                                                    <div className='top  p-3'>
                                                                        ...
                                                                        <div className='button-rate1 ' >
                                                                            <button className='btn btn-dark mb-2 mt-2'>Đánh giá</button>
                                                                        </div>
                                                                    </div>
                                                                    <div className='d-flex justify-content-center' >
                                                                        <button className='button-rate1 button-rate2 btn btn-dark mb-3 mt-2'>Đánh giá </button>
                                                                    </div>
                                                                </div>
                                                            )
                                                        })
