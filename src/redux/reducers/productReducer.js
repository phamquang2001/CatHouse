...
import { ADD_TO_CART, FETCH_ALL_PRODUCTS, DECREASE_CART, INCREASE_CART, DELETE_CART, ORDER_ONE, BUY_ONE, DELETE_CART_BUY_ONE, DECREASE_QUANTITY_BUYONE, COUNT_CART, GET_ALL_CATEGORY, DELETE_ALL_CART, SHOW_CART } from "../actions/productActions";
        ...
        case ORDER_ONE:
            ...
            state.cartProduct.map((item) => {
                action.payload.map((order, index) => {
                    ...
                    if (item.id * item.quantity * item.price !== order.cartId * order.quantity * order.price) {
